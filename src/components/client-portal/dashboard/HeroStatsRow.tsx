...
import { Wallet, CalendarDays, ChevronRight, TrendingUp, TrendingDown, Flame } from 'lucide-react';
...
export function HeroStatsRow({ period = 30 }: HeroStatsRowProps) {
  ...
  const { data: streak, isLoading: streakLoading } = useClientStreak(clientId ?? undefined);
  ...
  return (
    ...
    >
      ...
      {/* Streak Card */}
      <Card className="relative overflow-hidden bg-gradient-to-br from-warning/10 via-warning/5 to-transparent border-warning/20 h-full">
        <CardContent className="p-3">
          <div className="flex flex-col gap-1.5">
            ...
            <div>
              ...
              {streakLoading ? (
                ...
                <div>
                  <p className="text-base font-bold leading-tight">
                    {streak?.currentStreak ?? 0} {(streak?.currentStreak ?? 0) === 1 ? 'týden' : (streak?.currentStreak ?? 0) < 5 ? 'týdny' : 'týdnů'}
                  </p>
