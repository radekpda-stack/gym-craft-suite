...
import { Search, Dumbbell, Heart, Zap, Clock, Star, Activity, Plus, ExternalLink } from 'lucide-react';
...
export function ExerciseSearchCommand({ trigger, onQuickLog }: ExerciseSearchCommandProps) {
  ...
  const handleSelect = useCallback((exerciseId: string) => {
    if (onQuickLog) {
      // Show action menu instead of immediately navigating
  ...
  const handleQuickLog = useCallback((exerciseId: string, exerciseName: string) => {
    setOpen(false);
    ...
    <>
      {trigger ? (
        ...
        <div className="flex flex-col">
          ...
          {onQuickLog && (
            <span className="text-[10px] text-primary/70">Klikni pro zápis nebo detail</span>
          )}
        </div>
          <kbd className={cn(
          ...
          {recentExercises.length > 0 && (
            <CommandGroup heading="Nedávno použité">
              {recentExercises.map((recent) => {
                ...
                const isActionOpen = actionMenuId === recent.exercise_id;
                ...
                return (
                  ...
                  >
                    ...
                    {/* Inline action menu when onQuickLog is set */}
                    {isActionOpen && onQuickLog && (
                      <div className="flex border-t border-border/40 w-full">
                        <button
                          onClick={(e) => { e.stopPropagation(); handleQuickLog(recent.exercise_id, exercise.name_cs || exercise.name); }}
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); handleViewDetail(recent.exercise_id); }}
            ...
            <>
              ...
              <CommandGroup heading="Oblíbené">
                {favoriteExercises.map((exercise) => {
                  ...
                  const config = CATEGORY_CONFIG[catType];
                  const Icon = config.icon;
                  const isActionOpen = actionMenuId === exercise.id;
                  ...
                  return (
                    ...
                    >
                      ...
                      {isActionOpen && onQuickLog && (
                        <div className="flex border-t border-border/40 w-full">
                          <button
                            onClick={(e) => { e.stopPropagation(); handleQuickLog(exercise.id, exercise.name_cs || exercise.name); }}
                            className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-semibold text-primary hover:bg-primary/10 transition-colors"
                          >
                            <Plus className="w-3.5 h-3.5" />
                            Zapsat výkon
                          ...
                          <div className="w-px bg-border/40" />
                          <button
                            onClick={(e) => { e.stopPropagation(); handleViewDetail(exercise.id); }}
                            className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs text-muted-foreground hover:bg-muted/50 transition-colors"
                          >
                            <ExternalLink className="w-3.5 h-3.5" />
                            Detail cviku
