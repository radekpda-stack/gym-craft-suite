...
import { FileText, Upload, Loader2, CheckCircle, AlertTriangle, Package, Receipt } from 'lucide-react';
...
export function InvoiceImportDialog({ trigger }: InvoiceImportDialogProps) {
  ...
  const someSelected = state.items.some(i => i.selected);
          ...
          {(state.status === 'ready' || state.status === 'importing') && (
            <>
                ...
                <div className="p-3 rounded-lg bg-secondary/30 border border-border/50 mb-4">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-medium">
                      Rozpoznáno {state.items.length} položek
                    </span>
                  </div>
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                />
                <Label className="text-sm cursor-pointer" onClick={() => toggleAllItems(!allSelected)}>
                  Vybrat vše ({state.items.length} položek)
                </Label>
              </div>
