...
export function StockManagement() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (debouncedSearch) {
      ...
      result = result.filter(p => normalizeText(p.name).includes(searchNormalized));
    }
    switch (activeFilter) {
      case 'low_stock':
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        case 'name_asc': return a.name.localeCompare(b.name, 'cs');
        case 'name_desc': return b.name.localeCompare(a.name, 'cs');
  ...
  const getKindIcon = (productKind: string) => {
    switch (productKind) {
      ...
      case 'credit_topup': return <CreditCard className="w-5 h-5" />;
  ...
  const handleBulkArchive = async () => {
    try {
      await Promise.all(
        selectedProducts.map(p => updateProduct.mutateAsync({ id: p.id, is_active: false }))
      );
      toast({ title: `${selectedProducts.length} položek archivováno` });
      setSelectedIds(new Set());
  ...
  const handleBulkActivate = async () => {
    try {
      await Promise.all(
        selectedProducts.map(p => updateProduct.mutateAsync({ id: p.id, is_active: true }))
      );
      toast({ title: `${selectedProducts.length} položek aktivováno` });
      setSelectedIds(new Set());
    } catch { /* handled by mutation */ }
  };

  const handleBulkDelete = async () => {
    if (!confirm(`Opravdu smazat ${selectedProducts.length} položek? Tato akce je nevratná.`)) return;
    try {
      await Promise.all(
        selectedProducts.map(p => deleteProduct.mutateAsync(p.id))
      );
      toast({ title: `${selectedProducts.length} položek smazáno` });
      setSelectedIds(new Set());
    } catch { /* handled by mutation */ }
  };

  const handleBulkPriceApply = async (changes: { id: string; price: number }[]) => {
    try {
      await Promise.all(
        changes.map(c => updateProduct.mutateAsync({ id: c.id, price: c.price }))
      );
      toast({ title: `Ceny aktualizovány pro ${changes.length} položek` });
      setSelectedIds(new Set());
    } catch { /* handled by mutation */ }
  };
      ...
      {filteredProducts.length > 0 && (
        <div className="flex items-center gap-3 px-4 py-2 rounded-lg bg-secondary/30">
          ...
          <span className="text-xs text-muted-foreground">
            {selectedIds.size > 0 
              ? `Vybráno ${selectedIds.size} z ${filteredProducts.length}`
              : 'Vybrat vše'
            }
          </span>
        </div>
        ...
        {selectedIds.size > 0 && (
          ...
          >
            <div className="flex items-center gap-2 px-4 py-3 rounded-2xl bg-card/95 backdrop-blur-xl border border-border shadow-2xl">
              <Badge variant="secondary" className="mr-1">{selectedIds.size}</Badge>
              ...
              <Button size="sm" variant="outline" className="gap-1.5" onClick={handleBulkActivate}>
