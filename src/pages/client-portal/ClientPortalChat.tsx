...
export default function ClientPortalChat() {
  ...
  const { data: messages, isLoading } = useChatMessages(clientId ?? undefined, trainerId);
  ...
  useEffect(() => {
    if (clientId && trainerId && messages && messages.length > 0) {
      ...
      const hasUnread = messages.some(m => !m.isRead && m.senderType === 'trainer');
      if (hasUnread) {
  ...
  if (!trainerId) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <h2 className="text-lg font-medium">Chat není dostupný</h2>
        <p className="text-muted-foreground text-sm">
          Kontaktujte prosím svého trenéra jiným způsobem.
        </p>
      </div>
    );
  }
        ...
        <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center py-12">
              ...
              <h3 className="font-medium text-lg">Zatím žádné zprávy</h3>
              <p className="text-muted-foreground text-sm mt-1">
                Napište svému trenérovi první zprávu
              </p>
            </div>
          ) : (
            <>
