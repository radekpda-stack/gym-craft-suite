-- Fix uuid = text comparison error in rpc_process_sale
-- The issue is comparing (d->>'productId')::uuid with v_product_id 
-- when productId in JSON might not be a valid UUID or needs text comparison
...
BEGIN
  ...
  LOOP
    ...
    -- FIX: Compare text to text instead of trying to cast JSON value to UUID
    IF v_product.kind = 'inventory' THEN
      SELECT 
        (d->>'type')::text,
        (d->>'value')::numeric
