-- Fix rpc_complete_training_session: align with schema (credit_transactions.group_id, clients.credit_balance)
...
BEGIN
  ...
  LOOP
    ...
    -- Get individual payment method from participant, fallback to global
    ...
    ELSE
      -- Individual client balance (cached)
  ...
  -- Final session status: if any cash -> pending_payment else completed
