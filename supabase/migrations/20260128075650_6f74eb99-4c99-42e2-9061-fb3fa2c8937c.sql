-- Fix: when all participants pay with credit, payment_status should be 'paid_credit' not 'pending'
...
BEGIN
  ...
  LOOP
    ...
    -- Track payment methods that require manual confirmation
  ...
  -- Determine final status and payment_status
  -- If any participant uses cash/card/transfer/later -> needs manual confirmation
  IF v_has_cash_payment OR v_has_card_payment OR v_has_transfer_payment OR v_has_later_payment THEN
    v_final_status := 'completed';  -- Session is done, but payment needs confirmation
    v_final_payment_status := 'pending';
  ELSE
    -- All participants paid with credit -> fully completed
