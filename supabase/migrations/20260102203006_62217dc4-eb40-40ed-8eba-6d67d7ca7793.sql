-- Update rpc_process_sale to include XP calculation and award
...
BEGIN
  ...
  LOOP
    ...
    IF v_product.kind = 'inventory' THEN
      ...
      WHERE (d->>'productId')::uuid = v_product_id
      LIMIT 1;
  ...
  -- Award XP to client if applicable
  IF v_xp_earned > 0 AND v_client_id IS NOT NULL THEN
    PERFORM add_client_xp(v_client_id, v_xp_earned, 'sale');
  END IF;
