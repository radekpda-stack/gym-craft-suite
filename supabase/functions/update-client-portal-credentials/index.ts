...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { client_id, new_email, new_password } = await req.json();
    ...
    if (!new_email && !new_password) {
      return new Response(
        JSON.stringify({ error: 'Nebyl zadán email ani heslo k aktualizaci' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (accountError || !clientAccount?.auth_user_id) {
      return new Response(
    ...
    // Prepare update object for auth user
    ...
    const accountUpdate: { portal_password?: string; last_password_reset_at?: string; updated_at: string } = {
